...
The profiles in `apparmor.d` must not be used directly. They need to be prebuilt (by running `just complain`). This page documents all possible prebuild tasks. It is not intended to be read by end users, and it is only targeted at developers and maintainers.
